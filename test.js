...
async function run() {
  ...
  // Потолок должен быть выше частоты будильника, иначе из-за плавающего времени
  // старта он изредка резал бы законное обновление; и не выше 12 — это предел кода
  check('часовой потолок выше частоты будильника', cap > 6, `потолок ${cap} при 6 запусках в час`);
  check('часовой потолок не выше 12', cap <= 12, `сейчас ${cap}`);
